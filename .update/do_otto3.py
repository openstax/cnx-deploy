...
REX_FILEPATH = here.resolve().parent / 'roles/webview/files/etc/nginx/uri-maps/rex-rex.map'
...
def rex_uri(book, page):
    ...
    with open("rex_uri.txt", "a") as myfile:
        myfile.write(uri)
...
def cnx_uri_regex(book, page):
    if page is None:
        uri_regex = f"/contents/{book['short_id']} \n"
    else:
        uri_regex = f"/contents/{book['short_id']}:{page['short_id']} \n"

    with open("cnx_uri.txt", "a") as myfile:
        myfile.write(uri_regex)
...
def generate_cnx_uris_for_rex_books(rex_host):
    ...
    """
    # test URL (qa or staging)
    init_url = "https://qa.cnx.org"

    # current/future rex URL to compare against
    rex_base_url = "https://rex-web.herokuapp.com/"
    opx_base_url = "https://openstax.org/"
    ...
    try:
        for book in release_data['books']:
            ...
            for uri in generate_cnx_uris(book):
                ...
                # full URL
                full_cnx_url = init_url + uri

                if not full_cnx_url:
                    break
                else:
                    # opens the webpage, gets its current URL and compares to the expected URL (rex-web.herokuapp.com)
                    driver.get(full_cnx_url)
                    curr_url = driver.current_url
                    assert rex_base_url in curr_url

                    final_url = curr_url.replace("https://rex-web.herokuapp.com", "https://openstax.org")
                    print("full_openstax_url: ", final_url)
                    print("full_cnx_url     : ", full_cnx_url)


                if not final_url:
                    break
                else:
                    # opens the webpage, gets its current URL and compares to the expected URL (openstax.com)
                    driver.get(final_url)
                    final_curr_url = driver.current_url
                    assert opx_base_url in final_curr_url

            print("")
            print("----- COMPLETED BOOK/PAGES:", book)
            print("")
