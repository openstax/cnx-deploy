...
def rex_uri(book, page):
    ...
    with open("rex_uri5.txt", "a") as myfile:
        myfile.write(uri)
...
def cnx_uri_regex(book, page):
    ...
    with open("cnx_uri3.txt", "a") as myfile:
...
def generate_cnx_uris(book_id):
    ...
    for node in nodes[1:]:  # skip the book
        # Non-versioned URIs
        m = f"/contents/{book_id}:{node['id']}/{node['slug']} \n"
        #mm = f"/contents/{book_id}:{node['short_id']}/{node['slug']} \n"
        #mmm = f"/contents/{book_node['short_id']}:{node['id']}/{node['slug']}"
        #mmmm = f"/contents/{book_node['short_id']}:{node['short_id']}/{node['slug']}"
        ...
        for i in range(len(m)):
            mfull = "https://qa.cnx.org"+m

        with open("rex_cnx13.txt", "a") as myfile:
            myfile.write(mfull)

        import urllib.request
        f = open('rex_cnx13.txt', 'r')
        # considering each url in a new line...
        while True:
            URL = f.readline()
            if not URL:
                break
            mycontent = urllib.request.urlopen(URL).read()


        #with open("rex_cnx1.txt", "a") as myfile:
        #    myfile.write(mm)
